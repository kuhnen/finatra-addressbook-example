...
trait AddressBookDao extends DB[String, AddressBook] {
  implicit def dbId(addressBook: AddressBook) = addressBook.hashCode().toString
}
...
trait DB[K, V] {
  ...
  implicit def valueToKeyValue(v: V): (K,V) = (v, v)
  ...
  def update(value: V): Future[Unit] =  Future { data = data.updated(value, value) }
